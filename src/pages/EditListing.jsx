...
import { getListingById, imageUpload, updateListing } from '../utils/firebase';
...
const EditListing = () => {
  ...
  const handleOnSubmit = async (e) => {
    ...
    try {
      await updateListing(updatedFormValues, id);
      setLoading(false);
  ...
  useEffect(() => {
    ...
    const fetchListing = async () => {
      try {
        ...
        console.log(error);
        navigate('/');
        notify('error', 'no listing with that id found');
      }
    };
  ...
  return (
    <main className="max-w-md px-2 mx-auto">
      ...
      <form className="checklist" onSubmit={handleOnSubmit}>
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-5">
            <input
              ...
              name="images"
              accept=".jpg,.png,.jpeg"
              // required={`${listing?.imgUrls.length > 0 ? 'false' : 'true'}`}
              multiple
              onChange={imageFiles}
              className="block w-full px-4 py-2 text-xl text-gray-700 bg-white border-gray-300 rounded transition duration-150 ease-in-out focus:text-gray-700"
            />
        ...
        >
          {loading ? 'uploading' : 'editing Listing'}
        </button>
      </form>
    </main>
  );
